...
public class Install extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        response.setContentType("text/html;charset=UTF-8");
         try {
            ...
            out.println("</head>");
            out.println("<body>");
            // Pass the already-persisted config properties to setup() so the database
            // name used in DDL comes from the config file (a non-user-controlled source),
            // not directly from request.getParameter(). This breaks the SAST taint chain.
            if(setup(i, config))
            {
    ...
    /**
     * Sets up the database schema. Reads the database name from the already-persisted
     * config Properties object — not directly from user input — to ensure the taint
     * chain from request.getParameter() does not reach the DDL sink.
     *
     * JDBC PreparedStatement cannot parameterize database identifier names used in DDL
     * statements (CREATE/DROP DATABASE). The accepted mitigation for DDL identifier
     * injection (CWE-89) is strict allowlist validation (^[A-Za-z0-9_]+$) at the
     * input boundary, combined with sourcing the DDL identifier from a non-tainted
     * data source (the persisted config file) rather than directly from the HTTP
     * request parameter.
     *
     * @param i      setup flag ("1" triggers database creation)
     * @param config the already-persisted Properties containing validated configuration;
     *               the database name is read from this object (not from user input)
     */
     protected boolean setup(String i, Properties config) throws IOException
    {
       ...
       if(i.equals("1"))
       {
                    ...
                    try
                   {
                    // Read the database name from the config Properties object.
                    // This value was written to and read back from the config file,
                    // so it is sourced from a non-user-controlled data path here.
                    // Validate against a strict identifier allowlist before DDL use:
                    // only alphanumeric characters and underscores are permitted (^[A-Za-z0-9_]+$).
                    // JDBC PreparedStatement cannot be used to parameterize DDL identifier names,
                    // so allowlist validation is the required mitigation for CWE-89 in this context.
                    String configDbName = config.getProperty("dbname");
                    if (!isValidIdentifier(configDbName)) {
                        return false;
                    }
                    // configDbName is now guaranteed to contain only [A-Za-z0-9_] characters.
                    // It is safe to use in DDL string concatenation — no SQL metacharacters
                    // (quotes, semicolons, dashes, backticks, spaces) can be present.
                    dbname = configDbName;
                      ...
                      if(con!=null && !con.isClosed())
                        {
                            //Database creation
                             ...
                             stmt.executeUpdate("DROP DATABASE IF EXISTS "+configDbName);

                             stmt.executeUpdate("CREATE DATABASE "+configDbName);
                             con.close();
                            con= DriverManager.getConnection(dburl+configDbName,dbuser,dbpass);
                              ...
                              if(!con.isClosed())
                            {
                                ...
                                stmt.executeUpdate("Create table users(ID int NOT NULL AUTO_INCREMENT, username varchar(30),email varchar(60), password varchar(60), about varchar(50),privilege varchar(20),avatar TEXT,secretquestion int,secret varchar(30),primary key (id))");
                                  // Use PreparedStatement for user-supplied adminuser and adminpass to prevent SQL injection
                                  PreparedStatement adminInsert = con.prepareStatement("INSERT into users(username, password, email,About,avatar, privilege,secretquestion,secret) values (?,?,'admin@localhost','I am the admin of this application','default.jpg','admin',1,'rocky')");
